...
def model_HAN():
    embedding_layer = Embedding(max_words,150,input_length=max_senten_len, trainable=False)
...
file1=createList("E:/rad_phd/news/neg",suffix=".txt")

file2=createList("E:/rad_phd/news/pos",suffix=".txt")
fname1 = []
fname2 = []
cates = []
for n in file1:
...
sns.countplot(cates)
plt.xlabel('Label')
plt.title('number of news of each category')
plt.show()
le = LabelEncoder()
le.fit(cates)
...
print("unique words",len(np.unique(text)))
#exit(0)    
paras = []
labels = []
texts = []
...
checkpoint = ModelCheckpoint('best_model.h5', verbose=0, monitor='val_loss',save_best_only=True, mode='auto')
history = model.fit(x_train, y_train, validation_split=0.2, epochs=30, batch_size=16, callbacks=[checkpoint])
